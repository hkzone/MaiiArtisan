...
if (btnQuantity) {
  ...
  let activeImages = document.getElementsByClassName('product__active');

  for (let i = 0; i < thumbnails.length; i++) {
    thumbnails[i].addEventListener('mouseover', function () {
      if (activeImages.length > 0) {
        activeImages[0].classList.remove('product__active');
      }

      this.classList.add('product__active');
      document.getElementById('product__featured').src = this.src;
    });
  }

  let buttonRight = document.getElementById('product__slideRight');
  let buttonLeft = document.getElementById('product__slideLeft');

  buttonLeft.addEventListener('click', function () {
    document.getElementById('product__slider').scrollLeft -= 180;
  });
